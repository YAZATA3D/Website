...
var Module = {
	...
	UE4_indexedDBName: 'UE4_assetDatabase_YAZATA3D_Website', // this should be an ascii ID string without special characters that is unique to the project that is being packaged
	UE4_indexedDBVersion: 202411150640, // Bump this number to invalidate existing IDB storages in browsers.
};
...
Module.arguments = ['../../../YAZATA3D_Website/YAZATA3D_Website.uproject','-stdout',];
...
//	}
	...
	function withIndexedDB(db) {
		...
		var mainCompiledCode = fetchFromIndexedDB(db, 'wasmModule').then(function(wasmModule) {
			...
			return download(Module.locateFile('https://dl.dropboxusercontent.com/scl/fi/1dtd7of998xeeodfwccd0/UE4Game-HTML5-Shipping.wasm?rlkey=azjh8tf538dcazged4tde68vw&st=v0m9jr8v&dl=0'), 'arraybuffer').then(function(wasmBytes) {
		...
		// ----------------------------------------
		// MAIN JS
		var mainJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://rawcdn.githack.com/YAZATA3D/Website/refs/heads/main/UE4Game-HTML5-Shipping.js'), 'blob').then(function(data) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://rawcdn.githack.com/YAZATA3D/Website/refs/heads/main/YAZATA3D_Website-HTML5-Shipping.data.js'));
		var utilityJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://rawcdn.githack.com/YAZATA3D/Website/refs/heads/main/Utility.js')).then(addScriptToDom);
		var dataDownload =
/* // The following code would download and store the .data file as a Blob, which should be more efficient than loading an ArrayBuffer. However that seems to be buggy, so avoid it for now.
			fetchOrDownloadAndStore(db, Module.locateFile('YAZATA3D_Website-HTML5-Shipping.data')).then(function(dataBlob) {
				return readBlobToArrayBuffer(dataBlob).then(function(dataArrayBuffer) {
					Module['preloadedPackages'] = {};
					Module['preloadedPackages'][Module.locateFile('YAZATA3D_Website-HTML5-Shipping.data')] = dataArrayBuffer;
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('https://dl.dropboxusercontent.com/scl/fi/ahg3qjwu2218n6k6d1g21/YAZATA3D_Website-HTML5-Shipping.data?rlkey=196dt1ecx2yp9z3e5ektx0jn3&st=72oh0y3s&dl=0'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('https://dl.dropboxusercontent.com/scl/fi/ahg3qjwu2218n6k6d1g21/YAZATA3D_Website-HTML5-Shipping.data?rlkey=196dt1ecx2yp9z3e5ektx0jn3&st=72oh0y3s&dl=0')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
